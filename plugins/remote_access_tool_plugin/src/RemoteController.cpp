...
// Port used by the TCP server and injected into client config files.
// Defined once here so there is a single place to change it.
...
static std::string getLocalIP() {
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (saddr == "127.0.0.1") continue;
 
        // Prefer any vboxnet interface — that is the host-only network
        // shared with VirtualBox VMs. Fall back to the first other address.
        if (iface.find("vboxnet") != std::string::npos) {
            preferred = saddr;
            break;
        }
...
// timed_system
//
// Original busy-polled with usleep(100 ms), wasting CPU throughout the wait.
// Replaced with waitpid(WNOHANG) inside a nanosleep loop that backs off
// progressively, keeping CPU usage negligible while still catching fast exits
// quickly.
// ---------------------------------------------------------------------------
static int timed_system(const char* cmd, int timeout_sec) {
    ...
    struct timespec ts{ 0, 20'000'000 };  // start at 20 ms
    while (true) {
        int status;
        ...
        // Back off up to 100 ms to avoid unnecessary wakeups for slow commands
        if (ts.tv_nsec < 100'000'000)
            ts.tv_nsec = std::min(ts.tv_nsec * 2, (long)100'000'000);
    }
}

// Helper: write an entire buffer to fd, checking the return value.
...
void RemoteController::notifyStatusChange() {
    if (m_statusCallback) {
        m_statusCallback(getServerStatus());
    }
}
...
// ──────────────── Push configuration to clients (via SCP) ─────────────────────
...
void RemoteController::pushAgentBinaryToClients() {
    const std::string agentBinaryPath = "/etc/themis/plugins/remote_access_tool/client_agent";
    ...
    // --- Step 1: kill existing agents (parallel, wait for all) ---
    // Use std::thread instead of raw pthread for exception safety and RAII.
    {
    ...
    // --- Step 2: upload binaries (sequential SCP) ---
    for (const auto& client : m_clients) {
    ...
    // --- Step 3: launch agents in parallel, wait for each with timeout ---
    {
...
void RemoteController::stopTCPServer() {
    std::cout << "[RemoteController] Stopping TCP server..." << std::endl;
    if (m_tcpHandler) {
        m_tcpHandler->stop();
        m_tcpHandler.reset();
        std::cout << "[RemoteController] TCP server stopped" << std::endl;
    }
}
...
//
// Original used line.find(token) to locate where to split the text payload,
// which would produce the wrong result if a token appeared earlier in the
// line (e.g. a client ID matching a keyword).  Replaced with a byte-offset
// approach: after tokenising, the remainder is extracted by scanning past
// exactly N whitespace-separated words from the start of the string, so the
// split point is always correct regardless of token content.
// ---------------------------------------------------------------------------

// Skip past `n` whitespace-delimited words in `s` starting at `offset`,
// then skip any leading whitespace, returning the index of what follows.
...
void RemoteController::handlePluginCommand(const std::string& line) {
    ...
    if (cmd == "status") {
        ...
        // Format: msg <clientId> <text...>
        // tokens: [0]=msg [1]=clientId, rest=text
        if (tokens.size() < 3) {
            std::cout << "Usage: server msg <clientId> <text>\n";
            return;
        }
        // Text starts after "msg <clientId> "
...
void RemoteController::cmdRulesList(const std::string& clientId) {
    ...
    std::cout << "\n=== dbus_filters for client '" << clientId << "' ("
...
void RemoteController::uploadFile(const std::string& clientId, const std::string& localPath,
...
void RemoteController::downloadFile(const std::string& clientId, const std::string& remotePath,
...
std::string RemoteController::getServerStatus() const {
    ...
    if (m_tcpHandler) {
        auto connected = m_tcpHandler->getConnectedClients();
        buffer << "Currently connected: " << connected.size() << "\n";
        for (const auto& c : m_clients) {
            bool isConn = std::find(connected.begin(), connected.end(), c.getId()) != connected.end();
            buffer << "  • " << c.getDisplayName()
                   << (isConn ? " [CONNECTED]" : " [DISCONNECTED]") << "\n";
        }
    }
...
// ── Themis plugin public wrappers ─────────────────────────────────────────────
// Thin delegators so the D-Bus plugin (functions.cpp) can reach the private
// implementations without needing friend access or duplicating logic.

void RemoteController::executeCommandPublic(const std::string& clientId,
                                             const std::string& command) {
    executeCommand(clientId, command);
}

void RemoteController::executeCommandByTagPublic(const std::string& tag,
                                                  const std::string& command) {
    executeCommandByTag(tag, command);
}

void RemoteController::executeCommandOnAllPublic(const std::string& command) {
    executeCommandOnAll(command);
}

void RemoteController::uploadFilePublic(const std::string& clientId,
                                         const std::string& localPath,
                                         const std::string& remotePath) {
    uploadFile(clientId, localPath, remotePath);
}

void RemoteController::downloadFilePublic(const std::string& clientId,
                                           const std::string& remotePath,
                                           const std::string& localPath) {
    downloadFile(clientId, remotePath, localPath);
}

void RemoteController::pushConfigToClientPublic(const std::string& clientId) {
    pushConfigToClient(clientId);
}